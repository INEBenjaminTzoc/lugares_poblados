...
(
    (
      (
        (
          (
            (
              (
                sum(IF((`lp`.`cod_estado` = '1'), 1, 0)) + sum(IF((`lp`.`cod_estado` = '2'), 1, 0))
              ) + sum(IF((`lp`.`cod_estado` = '3'), 1, 0))
            ) + sum(IF((`lp`.`cod_estado` = '4'), 1, 0))
          ) + sum(
            (
              IF((`lp`.`cod_estado` = '5'), 1, 0)
              AND (`lp`.`estado` = 'Y')
      ...
      ) + sum(IF((`lp`.`cod_estado` = '6'), 1, 0))
    ) + sum(IF((`lp`.`cod_estado` = '7'), 1, 0))
  ) AS `Total2018`
FROM
  (
    (
